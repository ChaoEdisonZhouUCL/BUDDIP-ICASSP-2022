...
def conv_block_torch(layers, in_channel, filters, kernel_size, strides, pad, downsample_mode, activation):
    # block consists of: conv1D+BN+Activation
    if downsample_mode == 'stride':
        layers.append(nn.Conv1d(in_channels=in_channel, out_channels=filters, kernel_size=kernel_size, stride=strides,
                                padding=pad))
    else:
        layers.append(
            nn.Conv1d(in_channels=in_channel, out_channels=filters, kernel_size=kernel_size, stride=1, padding=pad))
...
class EDIP_torch_I(nn.Module):
    def __init__(self, input_shape, output_shape, num_filters_up, num_filters_down, filter_size_up, filter_size_down,
                 filter_size_skip, downsample_modes, strides, activation, padding, use_skip_module=False):
        super(EDIP_torch_I, self).__init__()
        '''
    ...
    '''
        ...
        for id, (n, k, s, downsample_mode) in enumerate(
            ...
            out_channel = n
            self.down_layers = conv_block_torch(self.down_layers, in_channel=in_channel, filters=out_channel,
                                                kernel_size=k,
                                                strides=s, pad=padding, downsample_mode=downsample_mode,
        ...
        for id, (n, k) in enumerate(zip(num_filters_up, filter_size_up)):
            in_channel = out_channel
            out_channel = n
            self.up_layers = conv_block_torch(self.up_layers, in_channel=in_channel, filters=out_channel, kernel_size=k,
                                              strides=1, pad=padding, downsample_mode='stride',
                                              activation=activation)
        ...
        if self.use_skip_module:
            ...
            out_channel = input_shape[-2]
            self.skip_layers = conv_block_torch(self.skip_layers, in_channel=in_channel, filters=out_channel,
                                                kernel_size=filter_size_skip,
                                                strides=1, pad=0, downsample_mode='stride',
                                                activation=activation)
        # output
        self.out_layers = nn.ModuleList()
        in_channel = input_shape[-2]
        out_channel = output_shape[-2]
        self.out_layers = conv_block_torch(self.out_layers, in_channel=in_channel, filters=out_channel, kernel_size=1,
                                           strides=1, pad=0, downsample_mode='stride',
                                           activation='')
